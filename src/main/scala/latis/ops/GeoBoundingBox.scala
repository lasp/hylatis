...
class GeoBoundingBox(lon1: Double, lat1: Double, lon2: Double, lat2: Double) extends Filter {
  ...
  def predicate(model: DataType): Sample => Boolean = {
    val p1 = Selection(s"ix >= $x1").predicate(model)
    val p2 = Selection(s"iy >= $y1").predicate(model)
    val p3 = Selection(s"ix < $x2").predicate(model)
    val p4 = Selection(s"iy < $y2").predicate(model)
